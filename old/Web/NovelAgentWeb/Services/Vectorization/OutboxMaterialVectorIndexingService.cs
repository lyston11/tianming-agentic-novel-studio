...
public sealed class OutboxMaterialVectorIndexingService : IOutboxMaterialVectorIndexingService
{
    ...
    private readonly ILogger<OutboxMaterialVectorIndexingService> _logger;

    public OutboxMaterialVectorIndexingService(
        ...
        ILogger<OutboxMaterialVectorIndexingService> logger)
    {
        _db = db;
    ...
    public async Task IndexMaterialAsync(string materialId, string userId, CancellationToken ct = default)
    {
        ...
        var chunks = await LoadIndexChunksAsync(document, content, materialId, ct);
        ...
        if (!string.IsNullOrWhiteSpace(material.ProjectId))
            deleteFilters["project_id"] = material.ProjectId;

        await _vectorStore.DeleteVectorsByFilterAsync(userId, deleteFilters, ct);
        ...
        foreach (var chunk in chunks)
        {
            ...
            vectors.Add(new VectorData
            {
                ...
                Vector = embedding,
                UserId = userId,
                ProjectId = material.ProjectId ?? string.Empty,
                SourceType = "material",
                SourceId = materialId,
                ...
                {
                    ["version_id"] = document?.Id ?? materialId,
                    ["content_document_id"] = document?.Id ?? string.Empty,
        ...
        _logger.LogInformation("Indexed material {MaterialId} with {Count} chunks", materialId, chunks.Count);
    }

    private async Task<List<VectorChunkSource>> LoadIndexChunksAsync(
