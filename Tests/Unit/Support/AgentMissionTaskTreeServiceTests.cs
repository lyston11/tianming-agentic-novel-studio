...
public class AgentMissionTaskTreeServiceTests
{
    ...
    public void Sync_GateFailedChapterQueuesRepairInsteadOfBlocking()
    {
        ...
        var action = new AgentTaskScheduler().BuildContinueAction(new AgentObservationContext
        {
        ...
        Assert.Equal("run-001", action.ToolCall.Arguments["runId"]);
    }

    [Fact]
    public void BuildContinueAction_UsesQueuedTaskWhenProjectContentFocusMatchesRun()
    {
        var context = BuildProjectContentFocusedSchedulerContext(
            focusedRunId: "run-006",
            queuedRunId: "run-006");

        var action = new AgentTaskScheduler().BuildContinueAction(context);

        Assert.NotNull(action?.ToolCall);
        Assert.Equal("ProduceChapter", action!.ToolCall!.Name);
        Assert.Equal("run-006", action.ToolCall.Arguments["runId"]);
    }

    [Fact]
    public void BuildContinueAction_DoesNotHijackProjectContentFocusWithDifferentQueuedRun()
    {
        var context = BuildProjectContentFocusedSchedulerContext(
            focusedRunId: "run-006",
            queuedRunId: "run-007");
    ...
    [Fact]
    public void BuildContinueAction_DoesNotHijackExplicitChapterTargetWithDifferentQueuedTask()
    {
        var context = new AgentObservationContext
        {
            UserMessage = "请修订第 6 章，不要继续第7章。",
            TurnIntent = new TurnIntent
            {
                Type = TurnIntentType.FreeChat,
                ReferencedChapterId = "chapter-006",
                RawMessage = "请修订第 6 章，不要继续第7章。"
            },
    ...
    [Fact]
    public void BuildContinueAction_UsesExplicitChapterTargetWhenQueuedTaskMatches()
    {
        var context = new AgentObservationContext
        {
            UserMessage = "请修订第 6 章。",
            TurnIntent = new TurnIntent
            {
                Type = TurnIntentType.FreeChat,
                ReferencedChapterId = "chapter-006",
    ...
    private static AgentObservationContext BuildProjectContentFocusedSchedulerContext(
        string focusedRunId,
        string queuedRunId) => new()
    {
        RecentObservations =
        {
            new AgentRuntimeObservation
            {
                ...
                Artifact = new AgentToolArtifact
                {
                    ...
                    RunId = focusedRunId,
                    NextHints = new[] { "基于该章节工作流 Run 重新生成修订稿" }
                }
            }
        },
        MissionPlan = new AgentMissionPlan
            ...
            SchedulerState = new AgentTaskSchedulerState
            {
                Tasks =
                {
                    new AgentScheduledTask
                    {
                        ...
                        NextAction = "ProduceChapter",
                        RunId = queuedRunId,
                        ChapterId = queuedRunId == focusedRunId ? "chapter-006" : "chapter-007"
                    }
                }
            }
        }
