...
测试场景已在实现计划中定义：`Docs/superpowers/plans/2026-06-11-tool-search-prompt-optimization.md` Task 2

**待测试场景**：
1. 新会话问候 - 验证不会过度调用 tool_search
2. 创建小说 - 验证能正确发现 Planning 工具
3. 阶段切换 - 验证能在需要时调用 tool_search 切换阶段
4. 同阶段连续操作 - 验证能合理使用缓存工具

**测试方法**：启动服务后在前端逐一测试上述场景，观察 LLM 的工具调用行为是否符合"需求驱动"的设计意图。
