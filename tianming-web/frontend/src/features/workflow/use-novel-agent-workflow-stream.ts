...
/**
 * New Workflow stream (per .trellis/spec/frontend/state-management.md):
 * validates streamKind/streamId and monotonic sequence, keeps the durable
 * cursor, ignores transient token deltas, and never mutates business state —
 * it only triggers the shared invalidate().
 */
