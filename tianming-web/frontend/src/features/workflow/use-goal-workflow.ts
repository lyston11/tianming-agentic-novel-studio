...
import { useNovelAgentWorkflowStream } from './use-novel-agent-workflow-stream';
import { useGoalProgressStream } from './use-goal-progress-stream';
...
/**
 * Single owner of Goal workflow server state. Both SSE streams (legacy goal
 * progress + new Workflow stream) only announce changes — they both call the
 * same invalidate() so React Query refetches the authoritative snapshot.
 */
