...
public sealed partial class AgentToolRegistry
{
        ...
        CancellationToken ct)
    {
        ...
        var runId = FirstNonEmpty(ResolveRunId(call, session), session.RuntimeRunId);
        if (!string.IsNullOrWhiteSpace(runId))
            await AppendBookValidationProgressEventsAsync(session, projectId, runId, report, message, suggestions, ct)
                .ConfigureAwait(false);
    ...
    private async Task AppendBookValidationProgressEventsAsync(
        AgentSession session,
        string projectId,
        string runId,
        ...
        CancellationToken ct)
    {
        ...
        var data = new
        {
            ...
            startChapterNumber = report.StartChapterNumber,
            endChapterNumber = report.EndChapterNumber,
            ...
            issueCodes = report.Issues
                ...
                .Take(12)
                .ToArray(),
            suggestions = suggestions.Take(8).ToArray()
        };
        ...
        await writer.AppendChapterStageAsync(
                new AppendChapterProductionEventRequest(
                    ...
                    Status: FirstNonEmpty(report.OverallStatus, "completed"),
                    Message: message,
        ...
        await AppendChapterRuntimeProgressEventAsync(
                ...
                FirstNonEmpty(report.OverallStatus, "completed"),
                message,
                "book_validation_report",
                projectId,
                data,
    ...
    private async Task<AgentToolExecutionResult> SearchCreativeKnowledgeAsync(AgentToolCall call, AgentSession session, CancellationToken ct)
    {
        ...
        var materialHits = await SearchProjectMaterialKnowledgeAsync(query, session, ct).ConfigureAwait(false);
        var externalHits = dbHits
            .Concat(materialHits)
            .ToList();
        ...
        if (externalHits.Count > 0)
        {
            ...
            foreach (var hit in externalHits)
            {
            ...
            result.Message = $"创意知识库命中 {result.Hits.Count} 条（SQLite/Qdrant/Redis/项目素材知识通路）。";
        }
    ...
    private async Task<List<CreativeKnowledgeHit>> SearchProjectMaterialKnowledgeAsync(string query, AgentSession session, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(session.UserId) ||
            string.IsNullOrWhiteSpace(session.ActiveProjectId) ||
        ...
        using var scope = _serviceProvider.CreateScope();
        var semanticSearch = scope.ServiceProvider.GetService<SemanticSearchService>();
        if (semanticSearch == null)
            return new List<CreativeKnowledgeHit>();

        var results = await semanticSearch.SearchInProjectAsync(
                session.UserId,
                session.ActiveProjectId,
                query,
                8,
        ...
        return results
            .Where(result => string.Equals(result.EntityType, "material", StringComparison.OrdinalIgnoreCase))
            .Where(result => !string.IsNullOrWhiteSpace(result.Content))
            .GroupBy(result => FirstNonEmpty(result.EntityId, result.ChunkId), StringComparer.OrdinalIgnoreCase)
            .Where(group => !string.IsNullOrWhiteSpace(group.Key))
            .Select(group =>
            {
                var best = group.OrderByDescending(result => result.Score).First();
                return new CreativeKnowledgeHit
                {
                    Entry = new CreativeKnowledgeEntry
                    {
                        Id = FirstNonEmpty(best.EntityId, best.ChunkId),
                        Category = CreativeKnowledgeCategory.ProjectUsedPattern,
                        Title = $"项目素材：{FirstNonEmpty(best.EntityId, best.ChunkId)}",
                        Content = best.Content,
                        Source = "ProjectMaterial"
                    },
                    Score = best.Score,
                    Reason = "Project material search"
                };
            })
            .Take(4)
            .ToList();
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            await AppendPendingPostCommitOutboxQueuedEventsAsync(session, runId, result, ct).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    private async Task AppendPendingPostCommitOutboxQueuedEventsAsync(
        AgentSession session,
        string runId,
        NovelAgentExecutionResult result,
        CancellationToken ct)
    {
        ...
        var outboxes = await db.OutboxEvents
            ...
            .Where(e =>
                ...
                e.ProjectId == session.ActiveProjectId &&
                e.RuntimeRunId == runId &&
                e.Status != "completed")
        ...
        {
            var alreadyRecorded = await db.ProductionEvents
                .AsNoTracking()
                .AnyAsync(e =>
                    e.RuntimeRunId == runId &&
                    e.ProjectId == session.ActiveProjectId &&
                    ...
                    e.ArtifactId == outbox.Id,
                    ct)
                .ConfigureAwait(false);
            if (alreadyRecorded)
                continue;

            var stage = ResolvePostCommitOutboxStage(outbox.EventType);
            var message = $"后台 outbox 已排队：{outbox.EventType}/{outbox.AggregateType}。";
            var data = new
            {
            ...
            await writer.AppendChapterStageAsync(
                    new AppendChapterProductionEventRequest(
                        ...
                        UserId: session.UserId,
                        ProjectId: session.ActiveProjectId,
                        ChapterId: result.Run.TargetChapterId,
                        PackageId: null,
    ...
    private static string ResolvePostCommitOutboxStage(string eventType)
    {
        if (string.Equals(eventType, "extract_chapter_continuity_facts", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(eventType, "finalize_chapter_commit_metadata", StringComparison.OrdinalIgnoreCase))
        {
            return NovelAgentProductionStages.FactsPersisted;
        }

        if (eventType.StartsWith("index_", StringComparison.OrdinalIgnoreCase) ||
            eventType.StartsWith("delete_", StringComparison.OrdinalIgnoreCase))
        {
            return NovelAgentProductionStages.IndexUpdated;
        }
